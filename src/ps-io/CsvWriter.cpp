...
void CsvWriter::Write(pcpp::RawPacket* rawPacket, pcpp::PcapLiveDevice* device, void* cookie)
{
...
void CsvWriter::Flush()
{
    ...
    for (auto& elem : m_data)
    {
        ...
        outputFile << "\n"
                   << srcIP << "," << dstIP << "," << srcPort << "," << dstPort << "," << elem.second.first << "," << elem.second.second;
    }
}

// csvPcapWriter private

std::optional<CollectData> CsvWriter::getDataKey(const pcpp::Packet& packet) const
{
    ...
    if (ipLayer == nullptr)
    {
        LOG_WARNING("Couldn't find IPv4 layer...");
        return std::nullopt;
    }
    ...
    {
        LOG_WARNING("Couldn't find TCP nor UDP layers...");
        return std::nullopt;
    }
    if (tcpLayer != nullptr)
    {
