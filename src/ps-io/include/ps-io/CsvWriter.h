...
struct CollectData {
  ...
  CollectData(pcpp::IPv4Address sIP, pcpp::IPv4Address dIP, uint16_t sPort,
              uint16_t dPort)
...
struct CollectDataHash {
  std::size_t operator()(const CollectData &data) const {
    uint64_t first = (std::hash<uint32_t>()(data.srcIP.toInt()) ^
                      std::hash<uint16_t>()(data.srcPort))
                     << 32;
    uint64_t second = (std::hash<uint32_t>()(data.dstIP.toInt()) ^
                       std::hash<uint16_t>()(data.dstPort));
    return first + second;
  }
};
